...
If you wish to contribute to development, whether to submit a fix or add a tool, simply pull this git repository and have at the Rails code! The repository contains a recent copy of the database stored using the *yaml_db* gem.  You can thus instantiate a populated database with:
  rake db:create
  rake db:data:load
...
== Contact
* Please freel free to contact the project lead, Kent Mewhort (kent@openissues.ca), with any comments or questions. For bug reports, prerably raise them directly through Github.